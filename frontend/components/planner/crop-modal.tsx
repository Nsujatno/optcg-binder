...
import { slotLabel } from "@/lib/planner";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur">
      <div className="grid max-h-[95vh] w-full max-w-6xl gap-4 overflow-auto rounded-[32px] border border-white/10 bg-slate-950 p-5 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="rounded-[28px] border border-white/10 bg-white/5 p-4">
          <div
            className="grid aspect-[16/10] overflow-hidden rounded-[24px] border border-white/10 bg-black/50"
            style={{
        ...
        <div className="space-y-4 rounded-[28px] border border-white/10 bg-white/5 p-4">
          ...
          <label className="block text-sm">
            <span className="mb-1 block text-slate-300">Horizontal slots</span>
            <input
              className="w-full"
              max={activeTemplate.cols - currentSlotPosition.col}
              min={1}
              onChange={(event) =>
                setCropDraft((current) =>
                  current
                    ? { ...current, colSpan: Number.parseInt(event.target.value, 10) }
                    : current,
                )
              }
              type="range"
            ...
            />
            <span className="text-xs text-slate-400">{cropDraft.colSpan}</span>
          </label>

          <label className="block text-sm">
            <span className="mb-1 block text-slate-300">Vertical slots</span>
            <input
              className="w-full"
              max={activeTemplate.rows - currentSlotPosition.row}
              min={1}
              onChange={(event) =>
                setCropDraft((current) =>
                  current
                    ? { ...current, rowSpan: Number.parseInt(event.target.value, 10) }
                    : current,
                )
              }
              type="range"
            ...
            />
            <span className="text-xs text-slate-400">{cropDraft.rowSpan}</span>
          </label>
